...
export async function POST(request: NextRequest) {
  try {
    ...
    const { launchId, task, productName, price, targetAudience, mainProblem, sequenceType, numEmails } = body
    ...
    const userPrompt = generateSalesPrompt(task, {
      productName,
      price,
      targetAudience,
      mainProblem,
    ...
    const message = await anthropic.messages.create({
      ...DEFAULT_CONFIG,
      system: AGENT_SALES_SYSTEM_PROMPT,
      messages: [
        {
          role: 'user',
          content: userPrompt,
