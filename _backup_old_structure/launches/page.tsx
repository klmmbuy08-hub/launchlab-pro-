  ...
  useEffect(() => {
    const fetchLaunches = async () => {
      try {
        ...
        setLaunches(sampleLaunches as any);
      } finally {
        setIsLoading(false);
      }
    };
          ...
          <CardContent className="p-0">
            {isLoading ? (
              <div className="flex items-center justify-center p-8">
                ...
                <span className="ml-2 text-sm text-zinc-500">Cargando lanzamientos...</span>
              </div>
            ) : launches.length === 0 ? (
              <div className="flex flex-col items-center justify-center p-8 text-center">
                <Rocket className="h-12 w-12 text-zinc-300 mb-2" />
                <p className="text-zinc-600 font-medium">No hay lanzamientos aún</p>
                <p className="text-xs text-zinc-500 mt-1">Crea tu primer lanzamiento para empezar</p>
                <Link href="/launches/new" className="mt-4">
                  <Button size="sm">
                    <Plus className="h-3.5 w-3.5" />
                    Crear lanzamiento
                  </Button>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-zinc-50">
                    {launches.map((launch) => {
                      ...
                      const launchDate = launch.launch_date
                      ...
                      return (
                        <tr key={launch.id} className="hover:bg-zinc-50">
                          ...
                          <td className="px-4 py-4">
                            <span className="text-xs text-zinc-500 capitalize">{launch.product_type}</span>
                          </td>
                          <td className="px-4 py-4">
                            <Badge variant={status?.variant || "secondary"}>{status?.label || launch.status}</Badge>
                          </td>
                          <td className="px-4 py-4 text-right text-zinc-700">
                            ${launch.price.toLocaleString("es-ES")}
                          </td>
                          <td className="px-4 py-4">
                          ...
                          <td className="px-4 py-4">
                            <div className="flex items-center gap-1">
                              <Link href={`/launches/${launch.id}`}>
                                <Button variant="ghost" size="icon" className="h-7 w-7 text-neutral-400 hover:text-primary-500">
